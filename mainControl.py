...
def count_switches(photon_count_array):
	...
	switch_idxs = []
	threshold = 10
	for idx in range(len(photon_count_array)-1):
		val_front = photon_count_array[idx+1]
		val_back = photon_count_array[idx]
		switch_condition = (val_back>threshold and val_front<threshold) or (val_back<threshold and val_front>threshold)
		if switch_condition:
			switch_count = switch_count + 1
			switch_idxs.append(idx)
	return switch_count, switch_idxs	
